...
function repaintScheduleDate(curDateLoc) {
    ...
    for (var i = 0; i < dates.length; i++) {
        ...
        dateContent += '<li class="nav-item" ><a id="schedule-date' + i + '" class="nav-link" href="#"  onclick="repaintScheduleDate(\'' + i + '\')">' + date + '</a></li>';
    }
...
function repaintScheduleBody(curDateLoc) {
    ...
    for (var i = 0; i < scheduleItems.length; i++) {
        bodyContent += "<tr>" +
            "<td>" + scheduleItems[i].startTime.substring(11, 16) + "</td>" +
            ...
            "<td><a class='btn btn-primary' href='/user/movieDetail/buy?id="+movieId+"&scheduleId="+scheduleItems[i].id+"'>选座购票</a></td>" +
            "</tr>";
    }
